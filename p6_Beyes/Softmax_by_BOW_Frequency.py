...
class Softmax_by_SGD:
    def __init__(self):
        self.theta = 0.1 * np.random.rand(WORD_TOTAL_NUM,TYPE_NUM) - 0.1
    ...
    def get_accuracy(self):
        # 工具变量
        totNum = 100  # total num
        rightNum = 0
        # 分别访问每个 train file
        for i in range(TYPE_NUM):
            test_data = TRAIN_DATA[i]  # Get_Data.read_a_file(FILE_PRE + '/test/' + FILE_NAME[i] + '.txt')
            #totNum += len(test_data)
            # 访问每个句子，对每个句子训练
            for j in range(totNum):#len(test_data)):
                # 将每个句子转化为 BOW 表示
